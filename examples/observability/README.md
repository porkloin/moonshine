...
- Watching streaming-session latency live (per-frame traces, per-stage
  histograms, GPU clock/busy %, VRAM, DMA-BUF cache size)
...
- `moonshine.gpu.sclk_mhz` — gauge, sampled at 1 Hz on AMD cards
- `moonshine.gpu.busy` — gauge, %
- `moonshine.vram.used` — gauge, bytes
